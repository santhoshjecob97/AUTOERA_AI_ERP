...
export const Customer360Page: React.FC = () => {
  ...
  const handleNextBestAction = () => {
    setActionSuccess("Action Proposal Generated: Personalized Diwali EV Upgrade Offer queued for Manager Approval (#AP-9921).");
      ...
      <div className="bg-white dark:bg-[#0c121e] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-2 text-xs text-slate-500 dark:text-slate-400">
                <span className="flex items-center gap-1.5"><Phone size={13} className="text-slate-400" /> {mockCustomer.phone}</span>
                <span className="flex items-center gap-1.5"><Mail size={13} className="text-slate-400" /> {mockCustomer.email}</span>
                <span className="flex items-center gap-1.5"><MapPin size={13} className="text-slate-400" /> {mockCustomer.city}</span>
                <span className="font-mono text-[11px] bg-slate-100 dark:bg-slate-800/80 px-2 py-0.5 rounded">GSTIN: {mockCustomer.gstin}</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800/80">
          <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
            ...
            <p className="text-lg font-bold text-slate-900 dark:text-white mt-0.5">{mockCustomer.clv}</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
            <span className="text-[11px] text-slate-400 uppercase font-semibold">Active Garage</span>
            <p className="text-lg font-bold text-slate-900 dark:text-white mt-0.5">{mockCustomer.vehicles.length} Vehicles</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
            <span className="text-[11px] text-slate-400 uppercase font-semibold">Churn Risk</span>
            <p className="text-lg font-bold text-emerald-500 mt-0.5">{mockCustomer.churnRisk}% (Low)</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
            <span className="text-[11px] text-slate-400 uppercase font-semibold">Satisfaction / CSI</span>
            <p className="text-lg font-bold text-orange-500 mt-0.5">{mockCustomer.sentiment} / 100</p>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'timeline' && (
        <div className="bg-white dark:bg-[#0c121e] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm animate-fade-in">
          <div className="relative border-l-2 border-slate-200 dark:border-slate-800 ml-4 pl-6 space-y-6">
            {mockCustomer.timeline.map((evt, idx) => (
